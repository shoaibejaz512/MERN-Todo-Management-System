...
const deleteCommentGroupTask = async (req, res) => {
  const session = await mongoose.startSession();
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      // If your schema contains deletedAt:
    ...
    // Broadcast to everyone currently inside this group task
    req.io
      .to(`task:${taskId}`)
    ...
    console.error(
      "Delete group task comment error:",
      error
    );
